...
- **ShowInMenu** – whether this playlist's full channel list appears in the CHANNELS menu.
- **GroupByFirstChar** – group channels under A/B/C… based on their name.
- **Groups** – optional semicolon-separated M3U `group-title` names to show as submenus for this playlist (for example `News;Sports` or `News ; Sports`). When set, only those groups are shown — and they appear even if **ShowInMenu** is unchecked, so you can surface just a few groups from a huge playlist. Leave it empty to keep the default behavior.
